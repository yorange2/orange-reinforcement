...
已知规则差异（路线图 §5 风险 G10，"迁移不是逐位复现"）：简版引擎回合
开始就抽牌（先手第 1 回合 4 张起手），orange-stone 按官方规则先手第 1
回合不抽（3 张起手）——后手补偿因此不同，两引擎的 P1 胜率整体下移，
但都在同一量级（2026-08 实测 80 局：简版 36% vs os 21%）。

阈值口径（N=48，二项 σ≈7pp，2026-08 实测校准）：
- 一致率 61% → 断言 > 45%（留 ~2σ 余量）
- P1 胜率：简版 34%、os 23% → 断言都在 [0.05, 0.55]，互差 ≤ 35pp
"""
