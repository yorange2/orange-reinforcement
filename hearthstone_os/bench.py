...
用法：
    python -m hearthstone_os.bench                              # 只比规则对手
    python -m hearthstone_os.bench --model hearthstone_os/models/agent.pt
    python -m hearthstone_os.bench --model ... --no-search      # 模型不带搜索
"""
...
from .search import TurnSearchAgent
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--no-search", action="store_true",
                        help="不给模型加整回合搜索（默认加，M3 回归基准的口径）")
    ...
    for path in args.model:
        ...
        if not args.no_search:
            contenders.append((
                f"{label} +搜索",
                TurnSearchAgent(agent.net, device=args.device, seed=args.seed),
            ))
    contenders.extend((name, BOTS[name](seed=args.seed)) for name in reversed(OPPONENTS))
