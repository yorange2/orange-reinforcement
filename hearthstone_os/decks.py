...
#: W0–W7（PR #79–#86）清偿 67 张后，2026-08-06 状态审计又补登记了 24 张此前
#: 未标记的既有简化卡（账本 §11；原 27 张，F-A8 过载修复后出账 3 张）——
#: `_load_debt_ids()` 现在提取 24 个 ID，卡池 = 414 − 24 债 − 21 衍生物 − 硬币
#: = 368（重编 wheel 后实测；鲜血小鬼 2026-08-06 落地）。若未来新增简化卡，按账本的维护约定登记：
...
def full_pool() -> list[str]:
    ...
    - 去掉引擎侧有简化债注释的卡（`_load_debt_ids()` 由 orange-stone 源码核对；
      2026-08-06 补登记 24 张，见 orange-stone 账本 §11）
